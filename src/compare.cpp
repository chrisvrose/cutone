...
void compareImages(std::string reference_filename,
    ...
    diffSingleChannel =
        (diffSingleChannel - minVal) * (255. / (maxVal - minVal));

    diff = diffSingleChannel.reshape(reference.channels(), 0);

    cv::imwrite("tonemap_differenceImage.png", diff);
    ...
    if (useEpsCheck) {
        ...
        checkResultsExact(
            referencePtr, testPtr,
            reference.rows * reference.cols * reference.channels());
    }

    std::cout << "PASS for errors" << perPixelError << " per pixel and "
              << globalError << " globally" << std::endl;
    return;
}
